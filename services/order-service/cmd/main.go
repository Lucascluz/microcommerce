...
func main() {
	port := utils.GetEnvOrDefault("PORT", "8084")
...
func startKafkaConsumer() {
	...
	for {
		...
		if string(m.Key) == "order-service" || string(m.Value) == "ping" {
			// Respond with service status
			resp := []byte(`{"status":"healthy", "service":"order-service", "timestamp":"` + time.Now().Format(time.RFC3339) + `"}`)
			ctx, cancel = context.WithTimeout(context.Background(), 5*time.Second)
			err = w.WriteMessages(ctx,
				kafka.Message{
					Key:   []byte("order-service"),
					Value: resp,
					Time:  time.Now(),
				},
			)
