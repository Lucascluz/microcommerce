...
func startKafkaConsumer() {
	...
	log.Println("Notifications service Kafka consumer started")
	...
	for {
		...
		if string(m.Key) == "notifications-service" || string(m.Value) == "ping" {
			// Respond with service status
			resp := []byte(`{"status":"healthy", "service":"notifications-service", "timestamp":"` + time.Now().Format(time.RFC3339) + `"}`)
			ctx, cancel = context.WithTimeout(context.Background(), 5*time.Second)
			err = w.WriteMessages(ctx,
				kafka.Message{
					Key:   []byte("notifications-service"),
					Value: resp,
					Time:  time.Now(),
				},
			)
