...
func startKafkaConsumer() {
	...
	log.Println("Payment service Kafka consumer started")
	...
	for {
		...
		resp := []byte(`{"status":"healthy", "service":"payment-service", "timestamp":"` + time.Now().Format(time.RFC3339) + `"}`)
		ctx, cancel = context.WithTimeout(context.Background(), 5*time.Second)
		err = w.WriteMessages(ctx,
			kafka.Message{
				Key:   []byte("payment-service"),
				Value: resp,
				Time:  time.Now(),
			},
		)
