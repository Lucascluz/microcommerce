...
func main() {
	port := utils.GetEnvOrDefault("PORT", "8086")
...
func startKafkaConsumer() {
	...
	log.Println("Sales service Kafka consumer started")
	...
	for {
		...
		if string(m.Key) == "sales-service" || string(m.Value) == "ping" {
			// Respond with service status
			resp := []byte(`{"status":"healthy", "service":"sales-service", "timestamp":"` + time.Now().Format(time.RFC3339) + `"}`)
			ctx, cancel = context.WithTimeout(context.Background(), 5*time.Second)
			err = w.WriteMessages(ctx,
				kafka.Message{
					Key:   []byte("sales-service"),
					Value: resp,
					Time:  time.Now(),
				},
			)
