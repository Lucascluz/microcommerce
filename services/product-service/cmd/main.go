...
func startKafkaConsumer() {
	...
	log.Println("product service Kafka consumer started")
	...
	for {
		...
		resp := []byte(`{"status":"healthy", "service":"product-service", "timestamp":"` + time.Now().Format(time.RFC3339) + `"}`)
		ctx, cancel = context.WithTimeout(context.Background(), 5*time.Second)
		err = w.WriteMessages(ctx,
			kafka.Message{
				Key:   []byte("product-service"),
				Value: resp,
				Time:  time.Now(),
			},
		)
