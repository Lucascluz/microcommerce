...
func main() {
	port := utils.GetEnvOrDefault("PORT", "8088")
...
func startKafkaConsumer() {
	...
	log.Println("Review service Kafka consumer started")
	...
	for {
		...
		if string(m.Key) == "review-service" || string(m.Value) == "ping" {
			// Respond with service status
			resp := []byte(`{"status":"healthy", "service":"review-service", "timestamp":"` + time.Now().Format(time.RFC3339) + `"}`)
			ctx, cancel = context.WithTimeout(context.Background(), 5*time.Second)
			err = w.WriteMessages(ctx,
				kafka.Message{
					Key:   []byte("review-service"),
					Value: resp,
					Time:  time.Now(),
				},
			)
