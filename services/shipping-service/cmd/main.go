...
func main() {
	port := utils.GetEnvOrDefault("PORT", "8085")
...
func startKafkaConsumer() {
	...
	log.Println("Shipping service Kafka consumer started")
	...
	for {
		...
		if string(m.Key) == "shipping-service" || string(m.Value) == "ping" {
			// Respond with service status
			resp := []byte(`{"status":"healthy", "service":"shipping-service", "timestamp":"` + time.Now().Format(time.RFC3339) + `"}`)
			ctx, cancel = context.WithTimeout(context.Background(), 5*time.Second)
			err = w.WriteMessages(ctx,
				kafka.Message{
					Key:   []byte("shipping-service"),
					Value: resp,
					Time:  time.Now(),
				},
			)
